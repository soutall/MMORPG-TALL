...
|---|---|---|---|
| **v1.38** | 23/09/2026 (hora local) | **JANELA DE HABILIDADES COM MOLDURA OFICIAL `SkillList.png` (1046×1504):** a arte oficial `imagem/HUD/SkillList.png` agora é a moldura da janela de Skills — técnica **"PNG por último"** (CAMADA 3, `pointer-events:none` na PNG), conteúdo dinâmico atrás (z-index 5) aparecendo pelos interiores transparentes da moldura, SEM fundo sólido e SEM redesenhar a interface. Container responsivo `min(90vw, calc(94vh*1046/1504))` + aspect-ratio 1046/1504 em PC e mobile. `renderizarSkills()` adaptada SEM duplicar lógica (mesmos `SKILLS_INFO`, `skillsNiveis`, upgrades, dano/cura/mana/cooldown/servidor): topo = 1 ícone de classe via `window.minhaClasse` (fotos `imagem/HUD/Perfil/*.png?v=perfil2`); esquerda = lista compacta por skill; direita = INFORMAÇÕES da skill selecionada (`window._skillSelecionada`); PRÓXIMO NÍVEL (lógica atual) e REQUISITOS (vazio se não houver); rodapé = barra de 8 slots; MELHORAR mantém `melhorarSkill(id)`. Geometria nos vãos transparentes (respeitando bandas douradas): pontos 7%/17.5%, lista 6.2%/20.5% h58%, info 49.8%/18% h34%, próximo 54.5%, requisitos 68%, barra 6%/87.2%, X 2.4%/3.4%. Drag & drop migrado para `#skills-png-wrap`/`#skills-drag-handle` (posição persistida). Cache-busters `SkillList.png?v=HUDSKILLS01`, `skills.css?v=144`, `skills.js?v=146`. Validado por render GDI+ (3 rodadas). Versão **v1.38** na login, HUD e `GAME_VERSION` | `index.html`, `skills.css`, `skills.js`, `dragdrop.css`, `dragdrop.js`, `imagem/HUD/SkillList.png`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
