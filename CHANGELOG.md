...
| **v1.52.0** | 25/09/2026 (hora local) | **SISTEMA FUNCIONAL DE CICLO DIA E NOITE COM AUTORIDADE DO SERVIDOR, RELÓGIO AO LADO DO MINIMAPA, TRANSIÇÕES SUAVES, MADRUGADA 100% BREU TOTAL (00:00 ÀS 04:00) E ILUMINAÇÃO DINÂMICA DE FOGUEIRAS, TOCHAS E LÂMPADAS:** (1) **🕒 Relógio ao lado do Minimapa:** Badge estilizado (`#minimap-relogio`) acoplado diretamente à esquerda do minimapa (`top: 0; right: 147px`), com ícone contextual (`☀️`, `🌅`, `🌇`, `🌆`, `🌙`, `🌑`), horário `HH:MM`, colorimetria dinâmica por fase e tooltip informativo com feedback por texto flutuante ao clicar/tocar. (2) **⚙️ Ciclo com Autoridade Server-Side:** Tempo sincronizado para todos os jogadores no `world_update` a cada tick (`Date.now()`); dias mais longos e noites mais curtas via constantes configuráveis (`cicloDiaSegundos: 120s` [06:00 às 18:00] e `cicloNoiteSegundos: 60s` [18:00 às 06:00], ciclo total de 3 min em modo acelerado para testes rápidos). (3) **🌅 Transições Suaves & Color Grading:** Entardecer progressivo iniciando às 18:00 com escurecimento gradual e matiz âmbar/carmesim até atingir a noite plena às 19:00 com matiz azul índigo (70% de escuridão); amanhecer suave das 05:00 às 06:00 clareando gradualmente com matiz dourado/rosa até pleno dia (0.0 de escuridão). (4) **🌑 Madrugada 100% Breu Total (00:00 às 04:00):** Durante a madrugada profunda (00:00 às 04:00), a escuridão atinge **100% de opacidade (`escuridao: 1.00`)**, ocultando 100% do cenário do mapa fora do raio de luz; o herói possui lanterna pessoal concentrada (~140px) e o radar do minimapa ganha máscara de escuridão periférica; transições suaves de 23:00 às 00:00 (0.70 → 1.00) e 04:00 às 05:00 (1.00 → 0.70). (5) **🔥 Iluminação Dinâmica com Recorte (Destination-Out):** Fogueiras dos acampamentos (`mapas.js`, `mapa_deserto.js`, `mapa_zona_zero.js`, editor), tochas, braseiros do Castelo, lâmpadas e postes furam a camada de escuridão com halos cintilantes (`pulsoFogo`), revelando o cenário nítido e conferindo utilidade tática essencial ao fogo e luz no game. (6) **⚡ Zero Overhead no Pleno Dia:** O loop de renderização encerra em `0.00ms` durante o dia (`escuridao <= 0.005`) mantendo 60 FPS estáveis no PC e Mobile. (7) **Validação:** Testes unitários completos do servidor (`teste_dia_noite.js`) e cliente (`teste_dia_noite_cliente.js`) 100% aprovados | `sistema_dia_noite.js`, `sistema_dia_noite_cliente.js`, `server.js`, `index.html`, `style.css`, `mapas.js`, `mapa_deserto.js`, `mapa_castelo.js`, `teste_dia_noite.js`, `teste_dia_noite_cliente.js`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
| **v1.51.1** | 25/09/2026 (hora local) | **FIX DO PAINEL ADMIN «EDITOR DE MONSTROS» QUE MORRIA A CADA `monstros_config` (TypeError `EFEITOS.forEach is not a function`):** (1) **O bug:** o `lintas()` do `mob-editor.js` tratava `window.EFEITOS` como se fosse um **array** e chamava `.forEach()` direto nele — mas o `debuffs.js` publica esse campo como um **mapa `id → definição`** (`{ nome, classe, icon, cor, desc, formato }`). Resultado: `TypeError: (window.EFEITOS || []).forEach is not a function` a cada mensagem `monstros_config`, o que **matava a montagem da aba "EDIT MOOB"** (`preencherImunes` → `carregarMob` → `refrescarEditorMob` → `abrirEditorMob`) — o painel ficava pela metade sempre que o admin abria a tela de spawn de monstros. (2) **Além do `forEach`, o formato dos campos também estava errado:** o editor esperava `e.id` e `e.emoji`, mas na definição **não existe campo `id`** (o id **é a chave** do mapa) e o ícone se chama **`icon`**, não `emoji` — então mesmo que fosse array, nenhum efeito teria id nem ícone. (3) **Correção:** novo helper `efeitoParaLista()` que percorre `Object.keys(window.EFEITOS)` e monta `{ id: chave, nome, emoji: def.icon, classe }`. Ele também **aceita array** e devolve **lista vazia** se o campo não existir, então o painel não quebra mais se a forma do `EFEITOS` mudar de novo. (4) **Não afetou o jogo:** `server.js` e o `debuffs.js` sempre usaram `EFEITOS[id]` (acesso por chave, correto) — o defeito era **exclusivo do painel admin**. Nenhum efeito, buff ou debuff mudou de comportamento. (5) **Validação: 17 asserções automatizadas, todas passaram** (`teste_mob_editor_efeitos.js`, novo) — roda o `lintas()` **real** contra o `EFEITOS` **real** do `debuffs.js` num DOM mínimo e confere que não estoura, separa debuffs/buffs, traz os **25 efeitos** do mapa, converte `icon` → `emoji`, usa a chave como `id`, inclui o `camuflagem` novo da v1.50.0, monta os checkboxes de imunidade com o `value` certo, e não quebra nem com array nem com o campo ausente | `mob-editor.js`, `index.html`, `teste_mob_editor_efeitos.js`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
...
| **v1.50.0** | 25/09/2026 (hora local) | **AJUSTE FINO DAS 6 CLASSES QUE O DASH v2 MUDOU (ESCUDO QUE GIRA + BAQUE, CAMUFLAGEM REAL DO SNIPER, LADINO ARRANCADAO, VFX POR CLASSE):** (1) **🛡️ GUERREIRO — o escudo DE VERDADE na mão agora GIRA com a espada (não era mais um VFX por cima):** antes o arco de bloqueio era desenhado como um brilho no chão e a plaquinha do escudo ficava sempre na mesma posição, dando a impressão de que ele não girava. Agora é o **escudo real, o da mão, que gira pelo mesmo ângulo da espada** (`translate(12,16) → rotate(angulo) → translate(-11,5)`, o mesmo pivô do braço). Para quem o vê, o estado vai no `world_update` (`escudoGuerreiroAtivo` + `escudoGuerreiroAng`) e o cliente registra por jogador (`window.guerreiroEscudoDe[id]`) — sem pacote por frame. (2) **🛡️ BAQUE DE ESCUDO 1x a cada 2 segundos (empurrão/estocada na direção da espada):** enquanto o escudo está erguido o Guerreiro **estoca a cada 2s** na direção em que está mirando: **30 de dano**, **empurra 80px** e **atordoa por 0,6s** num raio de **62px** a partir do corpo, no **mesmo arco de 66° do bloqueio**. Bate em slimes, **bosses tomam 35% e NÃO são empurrados**, e PvP só existe com o `pvpAtivo` ligado. O **1º baque vem em 1,2s** (e não 2s) de propósito: o escudo inteiro dura ~2s de stamina, então um baque em 2s competiria com o dreno zerando a stamina e quase nunca sairia. O sync do ângulo vai **limitado a ~8 pacotes/s** (só quando muda ≥0,20 rad, com no mínimo 120ms entre eles) para não pesar no ping. (3) **🌿 SNIPER — vestiu ROUPA DE CAMUFLAGEM de verdade, e a skill 4 SÓ funciona com ela (a moita dinâmica saiu):** o dash não teleporta, não cria moita e **não move o Sniper**: ele **veste o uniforme de camuflagem por 5 segundos** (calça, colete, capacete e gola alta com manchas opacas, e **pose deitado** enquanto está no mato), com **contagem regressiva `🎽 X.Xs` + barra de tempo** acima da cabeça (pisca nos últimos 1,5s). A **Camuflagem Natural (skill 4) só liga enquanto o uniforme está vestido**; sem ele o servidor recusa com `skill_aviso` motivo `sem_roupa` (e `fora_mato` se ele não estiver no mato), o botão fica apagado (`.camo-bloqueado`) e o efeito `camuflagem` deixou de ser infinito: **agora tem prazo = o que sobrar dos 5s**, então virou possível ver o contador. Como são **dois estados separados** — `snRoupaCamo` (vestido, vem do dash) e `snCamuflado` (escondido no mato, vem da skill 4) — **a camuflagem nunca sobrevive ao uniforme**. Tudo que era da moita dinâmica foi removido (renderizador, registro, eventos `action_sniper_moita_*` e sons). (4) **💚 CURANDEIRO — o escudo de área dura 10 segundos (era 8):** e a própria Curandeira passou a receber o evento `action_curandeiro_escudo_area` com `proprio: true`, senão a cúpula sumia no primeiro segundo para quem tinha levantado o escudo. (5) **👤 LADINO (Assassino) — Camuflagem Sombria de 2s com 90% de velocidade (era 80% por 1s):** invisível e com **+90% de velocidade por 2 segundos**, o "arranque" virou **só buff** (`soBuff: true`) — o servidor é quem manda o multiplicador autoritativo (`multVelocidade` + `multVelAte`), sem movimento roteirizado, então **client modificado não ganha velocidade extra**. O **PikeMan continua com o comportamento antigo** (200px / 1,8x / 1s). (6) **🎨 VFX por classe refeitos:** **Arqueiro** com 4 leques de folhas **verdes e opacos** (antes eram translúcidos); **Roqueiro** com uma **rajada de 7 notas musicais (♪♫♬) jogadas para trás** no dash, no lugar da mancha azul; **Guerreiro** com um **arco de brilho** no chão (o escudo em si é o da mão agora, então o VFX antigo duplicava o desenho); **Sniper** com a **onda verde** de queimar a roupa antiga na troca. (7) **🐛 BUG REAL CORRIGIDO NO CAMINHO — o Guerreiro FICAVA CONGELADO com o escudo erguido:** o pacote de **movimento do jogo real não tem campo `action`** (é só `{x, y, angulo, moving}`), e o filtro de ações bloqueadas do servidor checava `['movimento', ...].indexOf(data.action)`, que dava `-1` para `undefined` → **TODO movimento era descartado** enquanto o escudo estava de pé. Agora o filtro reconhece o pacote sem `action` que tem `x` como movimento livre; o resto das ações continua bloqueado. (8) **Validação: 169 asserções automatizadas, todas passaram** — **112 em E2E real** (`teste_dash.js`), **39 no cliente** (`teste_dash_cliente.js`, que extrai o bloco real do `index.html`) e **18 no visual** (`teste_dash_visual.js`, novo: roda `desenharGuerreiro`/`desenharSniper`/`desenharEscudoGuerreiro` contra um `ctx` falso que registra chamadas, para um método de canvas com nome errado aparecer no teste em vez de virar um buraco preto na tela) | `dash.js`, `dash-vfx.js`, `server.js`, `index.html`, `classes/guerreiro.js`, `classes/sniper.js`, `debuffs.js`, `sonoro.js`, `style.css`, `teste_dash.js`, `teste_dash_cliente.js`, `teste_dash_visual.js`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
| **v1.49.0** | 25/09/2026 (hora local) | **REESCRITA COMPLETA DO DASH: MECÂNICA DIFERENTE PARA CADA UMA DAS 12 CLASSES, COM COOLDOWN E CUSTO VALIDADOS NO SERVIDOR (ANTI-EXPLOIT) E DASH "DE VERDADE" (ANIMAÇÃO) NO LUGAR DO TELEPORTE SECO:** (1) **O QUE MUDOU DE FUNDO:** o dash deixou de ser "um teleport genérico de 160px com cooldown só no cliente" (dá para burlar spamming e teleportar de graça) e virou uma mecânica por classe, **inteiramente autoritativa no servidor**. Novo módulo **`dash.js` ISOMÓRFICO** (o *mesmo arquivo* é `<script>` no navegador e `require()` no servidor): cliente e servidor resolvem o caminho com o **mesmo algoritmo** (`resolverDestino`) e a **mesma curva** (`easeOut`), que é o que faz a predição do cliente e a autoridade do servidor convergirem sem o char "puxar para trás". (2) **DASH ANIMADO DE VERDADE (o pedido principal):** Arqueira, Roqueiro, Bárbaro, Ladino e PikeMan **correm de verdade** — o servidor interpola a posição no tick de 50ms, o cliente desenha, e o trajeto usa `easeOut` (arranca rápido e desacelera, com peso de personagem). Antes, "animar" o dash era só teletransportar e pintar um rastro; agora o servidor é quem **move** o char, então nem um client modificado teleporta. (3) **Guerreiro — ESCUDO FRONTAL SEGURADO (Espaço / botão, PC e Mobile):** levanta o escudo **na direção para onde a mira está apontando no momento** (não a direção de movimento), **não permite usar skill nem ataque básico** enquanto estiver assim, e **a stamina desce rápido até zerar** (50/s → cerca de **2 segundos** de escudo a partir do cheio; ao zerar o escudo cai sozinho). Reduz **70% do dano vindo da frente** num arco de ~66°; pelas costas o dano passa inteiro. Soltar (soltar o Espaço / o dedo) abaixa na hora. A passiva antiga de bloqueio por stamina foi **desligada enquanto o escudo está erguido** (somaria 91% de redução). (4) **Curandeiro — ESCUDO DE ÁREA NO GRUPO (sem teleporte):** gera um escudo de **8 segundos para os aliados do MESMO GRUPO** dentro de 160px (35% da vida máx de cada um), **consome TODA a stamina** e **atordoa a própria Curandeira por 5 segundos** (com 50% de redução de dano, para ser castigo e não morte instantânea). Empurra os inimigos ao redor 220px — **BOSSES NUNCA são empurrados**. (5) **Bárbaro — INVESTIDA (não teleporta):** corre ~400px em 0,5s com animação de investida, **atropelando**: causa dano nos slimes no caminho, empurra e atordoa (0,5s), e os **bosses tomam dano reduzido (35%) mas não são movidos**. (6) **Ladino e PikeMan — ARRANQUE (não teleporta):** ficam **invisíveis instantaneamente e ganham 80% de velocidade por 1 segundo**, correndo 200px; depois voltam ao normal e **reaparecem**. (7) **Summoner — o pet teleporta junto:** ao usar o dash, o Golem vai para o lado do dono na hora — **exceto durante o "Golem Sísmico"**, em que ele fica travado na animação da skill e não é teleportado. (8) **Mago — igual, só com efeito visual:** ganhou o anel de partículas roxo do teleporte. (9) **Arqueiro Astral (`arqueiro_arcano`) — BURACO NECRÓTICO:** aparece um mini portal negro com anéis roxos girando, o char entra e reaparece em outro lugar (240px). (10) **Sniper — MOITA DINÂMICA:** o dash **não move** o Sniper: **cria uma moita de planta no lugar**, que dura **1 minuto** (ou até ele criar outra, que **substitui** a anterior), com **ícone de contagem regressiva em cima** (vira vermelho e pisca nos últimos 10s) — é a camuflagem portátil dele. A moita some junto com o dono se ele desconectar. (11) **DroneMaster — inalterado** (continua o Escudo de Energia de 50% da vida máx, 10s de recarga). (12) **Validação de servidor e cliente, do jeito mais seguro possível:** cooldown e custo de stamina validados **no servidor** (`dashCdAte`), destino validado com o mesmo raycast de 10px do cliente (o dash para na parede, não atravessa), skills e ataque básico **bloqueados no servidor** enquanto o escudo está erguido (client modificado não ganha nada), e movimento travado durante o dash. **Ping e lag:** a predição é **instantânea e local** (o char responde na hora, sem esperar o servidor) e o servidor só manda **1 evento por dash** — nada de pacotes por frame; se a resposta chegar com caminho diferente (parede ou lag), o cliente se realinha na versão autoritativa em vez de "piscar". (13) **Validação automatizada: 121 asserções, todas passaram** — **81 em E2E real** (servidor no ar, login de verdade de **cada uma das 12 classes** por WebSocket: tipo de dash, alcance, stamina cobrada, cooldown bloqueando o 2º dash, escudo drenando a 50,2/s e bloqueando skill, escotilha de grupo zerando a stamina, moita de 60s, invisibilidade do arranque, pet do Summoner, e o **arqueiro medido quadro a quadro pelo `world_update` para provar que o movimento é progressivo e não um pulo**) + **40 no cliente** (o bloco real do `index.html` é extraído e roda contra um DOM mínimo: predição, teleporte, escudo, cooldown do servidor, recusa, colisão e os 15 VFX desenhando). Um bug real foi encontrado e corrigido no caminho: o dreno do escudo era **composto** (subtraía do valor já drenado a cada tick) e zerava a stamina em 0,5s em vez dos 2s previstos. Versão **v1.49.0** nos 3 pontos visuais | `dash.js`, `dash-vfx.js`, `server.js`, `index.html`, `teste_dash.js`, `teste_dash_cliente.js`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
| **v1.48.2** | 25/09/2026 (hora local) | **NOVO AJUSTE «MODO DE TELA» (JANELA / FULL JANELA) NA CONFIGURAÇÃO > VISUAL, VERSÃO PC APENAS, SALVO POR ID DO PERSONAGEM:** (1) **Nova opção na aba 🎨 VISUAL:** seletor **🖥️ MODO DE TELA (PC)** com as escolhas **Modo Janela** e **Modo Full Janela**, com dica informativa de que a escolha é salva por ID. O navegador não permite a uma aba comum redimensionar a janela do sistema, então a opção controla a **Fullscreen API**: *Full Janela* = tela cheia real (sem barra de navegador) e *Janela* = sai da tela cheia. (2) **PC apenas:** a linha inteira é **ocultada no mobile** (iPhone, iPad, Android e o iPadOS que se apresenta como "Macintosh" com multi-toque são detectados), porque lá a tela cheia é obrigatória para o jogo em paisagem. A detecção é por **plataforma**, não por tamanho de janela — assim o seletor não some quando o jogador escolhe *Modo Janela* e encolhe o navegador. (3) **Salvo por ID do personagem:** gravado em `mmorpg_visual_<id>` junto com os outros ajustes visuais (barras de Vida/Mana/XP), mais um espelho global `mmorpg_tela` usado no período anterior ao login (o id do personagem só chega no `init` do servidor). Os ajustes visuais são recarregados assim que o id chega, e o **teste confirma o isolamento**: 2 personagens no mesmo navegador têm preferências diferentes sem se misturar, e configs antigos sem o campo novo caem no padrão sem perder o que já estava salvo. (4) **Corrigido conflito com o auto-fullscreen:** o jogo **forçava tela cheia no primeiro clique em qualquer dispositivo** (`tentarHorizontalAutomatico`), o que tornaria o *Modo Janela* inútil. Agora esse automático **consulta a preferência**: no PC só entra em tela cheia se o jogador escolheu *Full Janela*; no mobile continua sempre entrando. (5) **Seletor nunca mente:** se o jogador sair/entrar da tela cheia por fora (Esc, F11), a preferência passa a refletir a realidade — apertar Esc grava *Modo Janela* e o próximo login já abre em janela. (6) **Validação:** 33 asserções automatizadas cobrindo detecção de plataforma (PC/iPhone/iPad/iPadOS-disfarçado/forced-mobile), ida e volta Janela↔Full Janela, idempotência das chamadas, persistência por ID e isolamento entre personagens, compatibilidade com configs antigos, sincronia com Esc/F11, portão do auto-fullscreen e exibição/escondimento do seletor — **todas passaram** | `config.js`, `config.css`, `index.html`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
| **v1.48.1** | 25/09/2026 (hora local) | **CORREÇÃO DAS TOCHAS DO CASTELO (SÓ DE PAREDE) + FIX DO PET DO SUMMONER CONGELADO NA DG:** (1) **Tochas do «Castelo Anda 1» — 60 → 57 fontes de luz, todas as tochas agora na parede:** Retiradas todas as tochas que ficavam no meio do caminho de caminhada. Das 42 tochas, **21 estavam soltas no corredor** (o teste antigo considerava "de parede" quem só tocava a parede na diagonal) e **2 pares eram DUPLICADOS na mesma posição** (duas tochas empilhadas em `(27,34)` e `(35,34)`). Agora cada tocha exige tile **sólido adjacente ortogonal** (N/S/L/O) — parede, pilar ou coluna: (a) as 4 tochas do **Corredor dos Pilares** saíram da linha central e foram montadas **nos pilares** (`(28,21)`, `(31,23)`, `(34,21)`) + `(37,20)` na parede; (b) 14 tochas foram **remontadas na parede mais próxima** de cada sala, preservando a iluminação original de todas as 10 áreas (nenhuma sala ficou escura); (c) 3 foram **removidas de vez** (2 na borda da Arena, já iluminada pelos 12 archotes, e 1 duplicata). Mantidos conforme decisão do usuário: 4 braseiros, 2 velas e os 12 archotes da Arena do Boss. **Nenhuma tocha no caminho de caminhada, zero duplicadas.** (2) **FIX: o Golem do Summoner não andava dentro da DG «Castelo Anda 1»:** Causa raiz — a função `posicaoPetValida` no servidor treatava os 9 biomas anteriores (green, desert, pantano, caverna, cidade, arena, cidadeperdida, testevisual, zonazero) mas **não tinha ramo para o castelo**; como `LARGURA_CASTELO === FIM_ZONA_ZERO === 82000`, **toda** posição do pet dentro da DG caía no `return false` final → o lacaio nascia e ficava **completamente congelado** (0px de deslocamento), sem conseguir andar, seguir o dono, atacar nem pousar do Salto. Adicionado o ramo do castelo com `colideCastelo`, `colisaoObjetosDoMapa('castelo')` e `colideObstaculosCustomizados('castelo')`. (3) **Colisão do castelo com raio:** `colideCastelo(x, y, raio?)` agora aceita raio opcional e testa os 4 pontos cardinais, para que o pet (entidade grande) não encoste o centro na parede; **sem raio o comportamento do jogador fica 100% inalterado** (compatibilidade preservada). Raio do pet no castelo é 12px em vez dos 24px globais porque a **passagem secreta da Câmara Secreta tem apenas 1 tile (40px)** — assim o Golem ainda atravessa. (4) **Validação real:** teste unitário do grid (0 tochas no caminho, 0 duplicadas, 10/10 salas com luz, colisão com raio, BFS de conectividade do spawn até a Arena do Boss e até a Câmara Secreta) + **teste E2E WebSocket de verdade** (servidor real, login real como summoner, teleporte real, caminhada real): o pet **nasce dentro do castelo e anda 93–160px** acompanhando o dono, sempre em tile livre e fora da parede. Bug reproduzido e confirmado desligando o ramo (0.0px) e corrigido religando (159px). Versão **v1.48.1** nos 3 pontos visuais | `mapa_castelo.js`, `server.js`, `index.html`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
...
| **v1.43.2** | 25/09/2026 (hora local) | **REMOÇÃO TOTAL DOS PICOS/ROCHAS ("ESPINHOS") + EXPANSÃO DE CABANAS E ACAMPAMENTOS NÔMADES AO REDOR DOS LAGOS:** (1) **Remoção de 100% das Montanhas e Rochas do Deserto:** A pedido do usuário, todas as formações de montanhas/rochas ('mont') foram completamente retiradas de todo o mapa do deserto, deixando o terreno 100% aberto, plano e amplo em dunas suaves; (2) **5 Oásis Cristalinos:** Adicionado o Oásis Escondido das Areias Rubras, totalizando 5 grandes lagos límpidos turquesa com palmeiras e marolas; (3) **12 Acampamentos e Cabanas Nômades (10 nas Margens dos Lagos):** Assentamentos completos com tendas de linho personalizadas (paletas cru, carmim, índigo, turquesa e dourado imperial), mastros, cordas, fogueiras com luz dinâmica, ânforas de água e tapetes orientais contornando as margens dos lagos preservando o leito de água; (4) **Otimização Dual PC & Mobile:** 60 FPS com Frustum Culling; versão **v1.43.2** nos 3 pontos visuais | `mapa_deserto.js`, `index.html`, `INFO_PROJETO.md`, `CHANGELOG.md`, `REGRAS_IA.md` |
...
Fix do painel admin «Editor de Monstros», que morria a cada `monstros_config`:

- **O bug**:
  - O `lintas()` do `mob-editor.js` chamava `.forEach()` direto em `window.EFEITOS`, como se fosse um **array**. Mas o `debuffs.js` publica esse campo como um **mapa `id → definição`**.
  - Isso estourava `TypeError: (window.EFEITOS || []).forEach is not a function` a cada mensagem `monstros_config`, matando a montagem da aba **«EDIT MOOB»** (`preencherImunes` → `carregarMob` → `refrescarEditorMob` → `abrirEditorMob`). O painel ficava pela metade sempre que o admin abria a tela de spawn de monstros.
- **O formato dos campos também estava errado**, o que teria quebrado mesmo se fosse array:
  - O editor esperava `e.id`, mas **não existe campo `id`** dentro da definição — o id **é a chave** do mapa.
  - O editor esperava `e.emoji`, mas o campo se chama **`icon`**.
- **Correção**: novo helper `efeitoParaLista()` que percorre `Object.keys(window.EFEITOS)` e monta `{ id: chave, nome, emoji: def.icon, classe }`. Ele **também aceita array** e devolve **lista vazia** se o campo não existir, então o painel não quebra mais se a forma do `EFEITOS` mudar de novo.
- **Não afetou o jogo**: `server.js` e o `debuffs.js` sempre usaram `EFEITOS[id]` (acesso por chave, correto). O defeito era **exclusivo do painel admin** — nenhum efeito, buff ou debuff mudou de comportamento.
- **Validação: 17 asserções automatizadas, todas passaram** (`teste_mob_editor_efeitos.js`, novo): roda o `lintas()` **real** contra o `EFEITOS` **real** do `debuffs.js` num DOM mínimo e confere que não estoura exceção, separa debuffs/buffs, traz os **25 efeitos** do mapa, converte `icon` → `emoji`, usa a chave como `id`, inclui o `camuflagem` novo da v1.50.0, monta os checkboxes de imunidade com o `value` certo, e não quebra nem com array nem com o campo ausente.

### v1.50.0 — 25/09/2026

Ajuste fino das 6 classes que o Dash v2 mexeu: escudo que gira de verdade + baque, camuflagem real do Sniper, arrancadão do Ladino e VFX por classe:

- **🛡️ Guerreiro — o escudo DE VERDADE na mão gira com a espada**:
  - Antes o arco de bloqueio era só um **brilho desenhado no chão**, e a plaquinha do escudo ficava sempre na mesma posição: parecia que ele não virava nunca.
  - Agora é o **escudo real, o da mão**, que gira pelo **mesmo ângulo da espada** — mesmo pivô do braço (`translate(12,16) → rotate(angulo) → translate(-11,5)`), só que no "modo escudo" o corpo assume a postura de defesa (braço da frente erguido, ombro baixo, escudo à frente) com um filete de brilho na borda de impacto.
  - **O que os outros jogadores veem** vem do `world_update` (`escudoGuerreiroAtivo` + `escudoGuerreiroAng`), indexado por jogador em `window.guerreiroEscudoDe[id]` — **nada de pacote por frame**.
  - O ângulo acompanha `p.angulo` (a mira) **a cada tick**, mas o sync é **limitado a ~8 pacotes/s**: só reenvia quando muda **≥ 0,20 rad** e com **no mínimo 120ms** entre dois envios.

- **🛡️ Guerreiro — baque de escudo 1x a cada 2 segundos**:
  - Enquanto o escudo está erguido, o Guerreiro **estoca a cada 2s na direção em que está mirando**: **30 de dano**, **empurra 80px** e **atordoa por 0,6s** num raio de **62px** a partir do centro do corpo, dentro do **mesmo arco de ~66° do bloqueio**.
  - Atinge slimes; **bosses tomam 35% do dano e NÃO são empurrados**; PvP só resolve com o `pvpAtivo` ligado.
  - **O 1º baque sai em 1,2s, não em 2s** — de propósito: o escudo inteiro dura ~2s de stamina, então um baque em 2s competiria com o dreno zerando a stamina e quase nunca sairia.
  - O dreno de stamina continua em **50/s**, então dá para ver ~1 baque por escudo cheio (2s de escudo = 1 baque no 1,2s; com o cooldown de 2s e a regeneração de 15/s, dá para encadear).

- **🌿 Sniper — ROUPA DE CAMUFLAGEM de verdade, e a skill 4 só funciona com ela**:
  - O dash **não teleporta, não cria moita e não move o Sniper**: ele **veste o uniforme de camuflagem por 5 segundos** (calça, colete, capacete e gola alta com manchas opacas, em pose deitado quando está no mato).
  - **Contagem regressiva `🎽 X.Xs` + barra de tempo** acima da cabeça, **pisca nos últimos 1,5s** para avisar que o uniforme está prestes a cair.
  - A **Camuflagem Natural (skill 4) só liga enquanto o uniforme está vestido**:
    - servidor recusa com `skill_aviso` motivo **`sem_roupa`** (ou `fora_mato`, se ele não estiver em uma moita do mapa);
    - o cliente já avisa antes de gastar MP, e o **botão fica apagado** (`.camo-bloqueado`);
    - o efeito `camuflagem` no HUD deixou de ser **infinito** (`∞`) e passou a mostrar o **tempo restante**.
  - **Dois estados separados**: `snRoupaCamo`/`snRoupaCamoAte` (vestido, vem do **dash**, muda o sprite e libera a skill 4) e `snCamuflado`/`snCamofladoAte` (escondido no mato, vem da **skill 4** — monstros perdem o alvo, ataque básico bloqueado, CD da skill 1 zera). **A camuflagem nunca sobrevive ao uniforme.**
  - **A moita dinâmica foi REMOVIDA por completo**: renderizador, registro, eventos `action_sniper_moita_criada`/`_sumiu` e o som antigo. O som virou `sniper_camuflagem`.
  - A roupa vencida **limpa o flag sozinha** no desenho (calculando o tempo antes de qualquer pixel), então um pacote perdido não deixa o Sniper vestido de camuflagem para sempre.

- **💚 Curandeiro — o escudo de área agora dura 10 segundos (era 8)**:
  - E a **própria Curandeira passou a receber o evento** `action_curandeiro_escudo_area` com `proprio: true` — sem isso a cúpula de quem levantou o escudo sumia no primeiro segundo.

- **👤 Ladino (Assassino) — Camuflagem Sombria de 2s com 90% de velocidade** (era 80% por 1s):
  - Invisível + **+90% de velocidade por 2 segundos**.
  - O "arranque" virou **só buff** (`soBuff: true`): o servidor é quem manda o **multiplicador autoritativo** (`multVelocidade` + `multVelAte`), sem movimento roteirizado e **sem `x0`/`x1` no confirme** — ou seja, **client modificado não ganha velocidade extra**.
  - O **PikeMan continua com o comportamento antigo** (200px / 1,8x / 1s).

- **🎨 VFX por classe refeitos**:
  - **Arqueiro**: 4 leques de folhas **verdes e opacos** (antes eram translúcidos e sumiam no fundo claro).
  - **Roqueiro**: rajada de **7 notas musicais (♪♫♬) jogadas para trás** no dash, no lugar da mancha azul borrada.
  - **Guerreiro**: só o **arco de brilho** no chão — o escudo em si é o da mão agora, então o VFX antigo duplicava o desenho.
  - **Sniper**: a **onda verde** da troca de roupa.

- **🐛 Bug real corrigido no caminho — o Guerreiro FICAVA CONGELADO com o escudo erguido**:
  - Causa raiz: o pacote de **movimento do jogo real não tem campo `action`** (é só `{x, y, angulo, moving}`), e o filtro de ações bloqueadas do servidor fazia `['movimento', ...].indexOf(data.action)`, que devolve `-1` para `undefined` → **todo movimento era descartado** enquanto o escudo estava de pé. O Guerreiro ficava travado no lugar, sem conseguir andar nem recuar.
  - Correção: o filtro reconhece o pacote **sem `action` mas com `x`** como movimento livre. **Todo o resto das ações continua bloqueado** (skills e ataque básico seguem barrados no servidor).
  - Aproveitado para testar: o teste do sync de ângulo dependia disso, e é a prova de que o caminho real de movimento continua vivo.

- **⚡ Ping e lag (mantido e reforçado)**:
  - Sync do ângulo do escudo com **throttle** (máx. ~8 pkt/s, só quando muda ≥0,20 rad, mínimo 120ms).
  - O VFX da troca de roupa usa a **posição do Sniper que se vestiu** (vem no evento), não a do jogador local — antes todo mundo veria a onda verde nascendo em si mesmo ao ver um aliado vestir.

- **Validação: 169 asserções automatizadas, todas passaram**:
  - **112 em E2E real** (`teste_dash.js`, porta 8097): sobe o servidor de verdade e confere, entre outras coisas, o **sync de ângulo do escudo** (manda o pacote de movimento real e mede o `action_guerreiro_escudo_ang`), o **baque** (primeiro entre 0,9s e 1,8s, na direção da espada, com posição e raio, e **no máximo 2 em 3,2s**), os `snapshot` com `escudoGuerreiroAtivo`/`escudoGuerreiroAng`, o **dreno de 50/s**, o **bloqueio de skill**, o **escudo de área de 10s**, a **invisibilidade de 2s com o multiplicador autoritativo** do Ladino, a Camuflagem Natural recusada sem a roupa, e a **remoção da moita**.
  - **39 no cliente** (`teste_dash_cliente.js`): o bloco **real** do `index.html` é extraído e roda contra um DOM mínimo; os **16 VFX** desenham sem erro.
  - **18 no visual** (`teste_dash_visual.js`, **novo**): `desenharGuerreiro`, `desenharSniper` e `desenharEscudoGuerreiro` rodam contra um **`ctx` falso que registra chamadas** — assim um método de canvas com nome errado (ou um branch novo quebrado) aparece no teste em vez de virar um buraco preto na tela do jogador. Cobre: escudo do dash girando com o ângulo certo, o registro por `id` para os outros jogadores, os **4 estados do Sniper** e a limpeza da roupa vencida.
  - A tabela de configuração do `dash.js` também é conferida direto (o Curandeiro **não desloca** — o 160 é o **raio** da área, não uma distância).

### v1.49.0 — 25/09/2026

Reescrita completa do Dash: mecânica diferente para cada uma das 12 classes, com cooldown e custo validados no servidor e dash "de verdade" (animação) no lugar do teleporte seco:

- **O que mudou de fundo**:
  - O dash deixou de ser "um teleport genérico de 160px com cooldown só no cliente" — que dava para burlar spamming e teleportar de graça — e virou uma mecânica por classe, **inteiramente autoritativa no servidor**.
  - Novo módulo **`dash.js` ISOMÓRFICO**: o *mesmo arquivo* é `<script>` no navegador e `require()` no servidor. Cliente e servidor resolvem o caminho com o **mesmo algoritmo** (`resolverDestino`, raycast de 10px) e a **mesma curva** (`easeOut`). É isso que faz a predição do cliente e a autoridade do servidor convergirem sem o personagem "puxar para trás".
  - **8 tipos de dash**: `escudo`, `energia`, `moita`, `area`, `teleporte`, `corrida`, `investida`, `arranque`.

- **⚡ Dash animado de verdade (o pedido principal)**:
  - Arqueira, Roqueiro, Bárbaro, Ladino e PikeMan **correm**: o servidor interpola a posição no tick de 50ms e o cliente desenha.
  - Antes, "animar" o dash era teletransportar e pintar um rastro por cima. Agora o servidor é quem **move** o personagem, então nem um client modificado teleporta de graça.
  - A curva `easeOut` arranca rápido e desacelera, dando peso de personagem.

- **🛡️ Guerreiro — Escudo frontal SEGURADO (Espaço / botão, PC e Mobile)**:
  - Levanta o escudo **na direção para onde a mira está apontando no momento** (não a direção de movimento).
  - **Não permite usar skill nem ataque básico** enquanto estiver assim.
  - **A stamina desce rápido até zerar**: 50/s → cerca de **2 segundos** de escudo a partir do cheio. Ao zerar, o escudo cai sozinho.
  - Reduz **70% do dano vindo da frente** num arco de ~66°; pelas costas o dano passa inteiro.
  - Soltar o Espaço / o dedo abaixa na hora (PC e Mobile).
  - A passiva antiga de bloqueio por stamina foi **desligada enquanto o escudo está erguido**, senão a redução somaria 91%.

- **💚 Curandeiro — Escudo de área no grupo (sem teleporte)**:
  - Escudo de **8 segundos** para os aliados do **MESMO GRUPO** dentro de 160px (35% da vida máx de cada um).
  - **Consome TODA a stamina** e **atordoa a própria Curandeira por 5 segundos**, com 50% de redução de dano (castigo, não morte instantânea).
  - Empurra os inimigos 220px — **BOSSES NUNCA são empurrados**.

- **💥 Bárbaro — Investida (não teleporta)**:
  - Corre ~400px em 0,5s com animação de investida, **atropelando**: causa dano nos slimes no caminho, empurra e atordoa (0,5s).
  - **Bosses tomam dano reduzido (35%) mas não são movidos.**

- **👤 Ladino e PikeMan — Arranque (não teleporta)**:
  - Ficam **invisíveis instantaneamente e ganham 80% de velocidade por 1 segundo**, correndo 200px.
  - Depois voltam ao normal e **reaparecem**.

- **👹 Summoner — o pet teleporta junto**:
  - Ao usar o dash, o Golem vai para o lado do dono na hora.
  - **Exceção: durante o "Golem Sísmico"** ele fica travado na animação da skill e **não** é teleportado.

- **🔮 Mago — igual, só com efeito visual**:
  - Ganhou o anel de partículas roxo do teleporte.

- **🕳️ Arqueiro Astral (`arqueiro_arcano`) — Buraco necrótico**:
  - Aparece um mini portal negro com anéis roxos girando, o char entra e reaparece em outro lugar (240px).

- **🌿 Sniper — Moita dinâmica**:
  - O dash **não move** o Sniper: **cria uma moita de planta no lugar**, que dura **1 minuto** (ou até ele criar outra, que **substitui** a anterior).
  - **Ícone de contagem regressiva em cima** da moita, que fica vermelho e pisca nos últimos 10s.
  - A moita some junto com o dono se ele desconectar.

- **🤖 DroneMaster — inalterado**:
  - Continua o Escudo de Energia (50% da vida máx, 10s de recarga).

- **🔒 Validação de servidor e cliente, do jeito mais seguro possível**:
  - **Cooldown e custo de stamina validados no servidor** (`dashCdAte`) — elimina o exploit de spamming.
  - Destino validado com o mesmo raycast de 10px do cliente: o dash **para na parede**, não atravessa.
  - **Skills e ataque básico bloqueados no servidor** enquanto o escudo está erguido — client modificado não ganha nada.
  - Movimento travado durante o dash.
  - Estado do dash é **por sessão** (nada é salvo em disco).

- **⚡ Ping e lag**:
  - A predição é **instantânea e local** (o char responde na hora, sem esperar o servidor).
  - O servidor manda **1 evento por dash** — nada de pacotes por frame.
  - Se a resposta chegar com caminho diferente (parede ou lag), o cliente se realinha na versão autoritativa em vez de "piscar".
  - Rede de segurança: se o servidor não confirmar em 400ms, a predição se auto-cancela — o char nunca fica "fantasma".

- **🐛 Bug encontrado e corrigido no caminho**:
  - O dreno de stamina do escudo do Guerreiro era **composto**: subtraía do valor *já drenado* a cada tick, criando um juro geométrico que zerava a stamina em **0,5s** em vez dos 2s previstos.
  - Corrigido com uma **base absoluta** (`staminaInicial` no instante em que o escudo foi erguido). Medido em seguida: **50,2/s** — exatamente o configurado.

- **Validação: 121 asserções automatizadas, todas passaram**:
  - **81 em E2E real** (`teste_dash.js`): sobe o servidor de verdade, faz login de **cada uma das 12 classes** por WebSocket e confere tipo de dash, alcance, stamina cobrada, cooldown bloqueando o 2º dash, escudo drenando a 50,2/s e bloqueando skill, escudo de grupo zerando a stamina, moita de 60s, invisibilidade do arranque, pet do Summoner, e o **arqueiro medido quadro a quadro pelo `world_update` para provar que o movimento é progressivo e não um pulo**.
  - **40 no cliente** (`teste_dash_cliente.js`): o bloco **real** do `index.html` é extraído e roda contra um DOM mínimo — predição, teleporte, escudo, cooldown do servidor, recusa, colisão e os **15 VFX** desenhando sem erro.
  - A tabela de configuração do `dash.js` também é conferida diretamente (tipo, distância, 1,8x, 1s, 5s, 60s, 70%, 50/s).

### v1.48.2 — 25/09/2026

Novo Ajuste «Modo de Tela» (Janela / Full Janela) na Configuração > Visual, Versão PC Apenas, Salvo por ID do Personagem:

- **Nova opção na aba 🎨 VISUAL**:
  - Seletor **🖥️ MODO DE TELA (PC)** com as escolhas **Modo Janela** e **Modo Full Janela**, mais uma dica informando que a escolha é salva por ID.
  - O navegador não permite que uma aba comum redimensione a janela do sistema, então a opção controla a **Fullscreen API**: *Full Janela* = tela cheia real (sem barra de navegador) e *Janela* = sai da tela cheia.

- **PC apenas**:
  - A linha inteira é **ocultada no mobile** (iPhone, iPad, Android e também o iPadOS que se apresenta como "Macintosh" com multi-toque).
  - No celular a tela cheia continua obrigatória, porque o jogo depende do modo paisagem.
  - A detecção é por **plataforma**, não por tamanho de janela: se fosse por tamanho, o seletor desapareceria justamente quando o jogador escolhe *Modo Janela* e encolhe o navegador.

- **Salvo por ID do personagem**:
  - Gravado em `mmorpg_visual_<id>`, junto com os outros ajustes visuais (barras de Vida / Mana / XP).
  - Espelho global `mmorpg_tela` usado no período anterior ao login, já que o id do personagem só é conhecido depois do `init` do servidor; os ajustes visuais são recarregados assim que o id chega.
  - **Isolamento verificado:** dois personagens no mesmo navegador têm preferências diferentes sem se misturar, e configs antigos (sem o campo novo) caem no padrão sem perder o que já estava salvo.

- **Corrigido conflito com o auto-fullscreen**:
  - O jogo **forçava tela cheia no primeiro clique em qualquer dispositivo** (`tentarHorizontalAutomatico`), o que tornaria o *Modo Janela* inútil.
  - Agora esse automático **consulta a preferência**: no PC só entra em tela cheia se o jogador escolheu *Full Janela*; no mobile continua sempre entrando.

- **O seletor nunca mente**:
  - Se o jogador sair ou entrar da tela cheia por fora (Esc, F11), a preferência passa a refletir a realidade: apertar Esc grava *Modo Janela* e o próximo login já abre em janela.

- **Validação**: 33 asserções automatizadas cobrindo detecção de plataforma (PC / iPhone / iPad / iPadOS disfarçado / forced-mobile), ida e volta Janela↔Full Janela, idempotência das chamadas, persistência por ID e isolamento entre personagens, compatibilidade com configs antigos, sincronia com Esc/F11, portão do auto-fullscreen e exibição/escondimento do seletor — **todas passaram**.

### v1.48.1 — 25/09/2026

Correção das Tochas do Castelo (Só de Parede) + Fix do Pet do Summoner Congelado na DG:

- **Tochas do «Castelo Anda 1» — 60 → 57 fontes, todas as tochas na parede**:
  - **Problema:** das 42 tochas, **21 estavam soltas no meio do caminho de caminhada** e **2 pares eram duplicados** (duas tochas empilhadas na mesma posição, em `(27,34)` e `(35,34)`). O teste anterior classificava como "de parede" qualquer tocha que apenas *tocasse* a parede na diagonal, o que mascarava several tochas no meio das salas.
  - **Regra nova:** tocha só é válida com tile **sólido adjacente ORTOGONAL** (N/S/L/O) — parede, pilar ou coluna.
  - **Corredor dos Pilares:** as 4 tochas da linha central foram montadas **nos pilares** (`(28,21)`, `(31,23)`, `(34,21)`) e uma na parede do corredor (`(37,20)`) — o corredor fica totalmente desimpedido.
  - **14 tochas remontadas** na parede mais próxima de cada sala, preservando a iluminação original de todas as 10 áreas do castelo (nenhuma sala ficou escura).
  - **3 removidas de vez:** 2 na borda da Arena (já iluminada pelos 12 archotes) e 1 duplicata.
  - **Mantidos conforme pedido:** 4 braseiros, 2 velas e os 12 archotes da Arena do Boss.
  - **Resultado: 0 tochas no caminho e 0 duplicadas.**

- **FIX: o Golem do Summoner não andava dentro da DG «Castelo Anda 1»**:
  - **Causa raiz:** `posicaoPetValida` (`server.js`) tratava os 9 biomas anteriores (green, desert, pantano, caverna, cidade, arena, cidadeperdida, testevisual, zonazero) mas **não possuía ramo para o castelo**. Como `LARGURA_CASTELO === FIM_ZONA_ZERO === 82000`, **toda** posição do pet dentro da DG caía no `return false` final.
  - **Efeito:** o lacaio nascia **completamente congelado** (0px de deslocamento) — não andava, não seguia o dono, não atacava e não conseguia pousar o Salto do Ogro.
  - **Correção:** adicionado o ramo do castelo com `colideCastelo`, `colisaoObjetosDoMapa('castelo')` e `colideObstaculosCustomizados('castelo')`.

- **Colisão do castelo com raio (`colideCastelo(x, y, raio?)`)**:
  - Com raio, testa também os 4 pontos cardinais, para que o pet (entidade grande) não encoste o centro na parede.
  - **Sem raio, o comportamento do jogador fica 100% inalterado** (compatibilidade preservada).
  - Raio do pet no castelo é **12px** em vez dos 24px globais porque a **passagem secreta da Câmara Secreta tem apenas 1 tile (40px)** — assim o Golem ainda consegue atravessá-la.

- **Validação executada**:
  - **Teste unitário do grid:** 0 tochas no caminho, 0 duplicadas, 10/10 salas com luz, colisão com raio (borda bloqueia, centro livre), BFS de conectividade do spawn até a Arena do Boss e até a Câmara Secreta, `node --check` sem erros.
  - **Teste E2E WebSocket real** (servidor de verdade, login como summoner, teleporte real, caminhada real): o pet **nasce dentro do castelo e anda 93–160px** acompanhando o dono, sempre em tile livre e fora da parede.
  - **Bug reproduzido e confirmado:** desligando o ramo do castelo o pet volta a 0.0px; religando anda 159px.

### v1.48.0 — 25/09/2026

Nova Dungeon «Castelo Anda 1» com 12 Salas Temáticas, Iluminação Dinâmica Realista nas Paredes e Arena Circular de Boss no Final:

- **Nova Dungeon de Castelo Medieval (2.200 × 1.800 px / 55×45 tiles de 40px)**:
  - Criado o módulo isomórfico `mapa_castelo.js` executável tanto no cliente (navegador) quanto no servidor Node.js.
  - Coordenadas de mundo fixadas em `X ∈ [82000, 84200]` e `Y ∈ [0, 1800]`, com `WORLD_WIDTH` mundial expandido para `84.200px`.
  - Terreno de pedra ardósia ancestral (`#2a2520`), juntas de argamassa, rachaduras aleatórias, alvenaria gótica com blocos individuais, chanfros superiores, mofo/umidade e sombreamento 2.5D.
- **12 Salas Temáticas Ricamente Detalhadas**:
  - **Câmara de Entrada**: Spawn inicial seguro (`82200, 900`), portal rúnico azul de retorno à Zona Zero (`82060, 900`), tochas em pilares e tapete carmesim.
  - **Sala dos Guardas**: Mesas, bancos rústicos, estantes de armas, manequins de armadura e braseiro central crepitante.
  - **Corredor dos Pilares**: Longa galeria com 6 pares de pilares góticos chanfrados com capitéis esculpidos e tochas ritmadas.
  - **Câmara de Tortura**: Paredes com correntes oscilantes de ferro, gaiolas suspensas oscilando suavemente, poças escuras no piso e névoa rasteira úmida.
  - **Câmara Secreta**: Sala oculta acessível por passagem secreta com armários, baús e teias de aranha ancestrais.
  - **Biblioteca das Sombras**: Estantes de livros embutidas nas paredes, escrivaninha antiga, pergaminhos e iluminação quente de velas agrupadas.
  - **Sala dos Troféus**: Escudos heráldicos fixados nas paredes de pedra e pedestais com taças de ouro e cálices de honra.
  - **Sala de Armas**: Manequins com armaduras completas de ferro, lanças e espadas empilhadas prontas para batalha.
  - **Sala do Trono**: Trono gótico monumental entalhado com brasão heráldico e almofada carmesim, colunas coríntias caneladas com capitéis florais, braseiros duplos com chamas volumétricas e tapete real com bordas douradas.
  - **Câmara das Correntes (Calabouço)**: Dezenas de elos de correntes penduradas do teto com oscilação orgânica, ossadas espalhadas e portões pesados de ferro forjado.
  - **Antessala do Boss**: Runas arcanas luminosas gravadas no piso em círculos concêntricos e iluminação cerimonial mística.
  - **Grande Arena Circular do Boss**: Ampla arena circular de raio 7 tiles (~560px de diâmetro), rosácea e mosaico estrelado central em pedra dourada/negra, 12 archotes em pilares ao redor do perímetro com chamas e brasas flutuantes, 8 conjuntos de correntes grossas nas paredes curvas, manchas de sangue de batalhas antigas e altar cerimonial no centro preparado para o spawn do Boss futuro.
- **Sistema de Iluminação Dinâmica Realista (60+ Fontes de Luz)**:
  - Tochas de parede com chamas vivas em camadas, núcleo quente e halos âmbar pulsantes (`sin(t)`).
  - Braseiros sobre tripés com chamas volumétricas ondulantes e brasas incandescentes.
  - Velas trêmulas agrupadas com pavio suave na Biblioteca e Antessala.
  - Archotes cerimoniais ao redor da Arena do Boss com partículas de brasas flutuando no ar.
  - Composição luminosa em modo `lighter` projetando gradientes radiais no piso e refletindo nas paredes de pedra.
- **Correntes com Física Pendular e Névoa Rasteira**:
  - Elos de ferro forjado desenhados individualmente com oscilação harmônica suave pelo vento que sopra dos subterrâneos.
  - Camadas de névoa rasteira translúcida cobrindo o chão da Câmara de Tortura e da Câmara das Correntes.
- **Integração Completa Multi-Plataforma (Dual PC & Mobile)**:
  - Portal rúnico de entrada no bioma glacial Zona Zero em `81900, 1000` (com vórtice dourado `desenharPortalCasteloEntrada`).
  - Botão temático de teleporte rápido no HUD (`🏯 Castelo Anda 1 (DG)`).
  - Autoridade de colisão física e limites no servidor (`colideCastelo` e `limitesMapaJogador`).
  - Integração no Editor de Colisões/Camadas (`colisoes_castelo.json` e `camadas_castelo.json`).
  - Radar e minimapa com bioma `🏯 Castelo Anda 1 (DG)` e fundo temático escuro `#140e0a`.
  - Versão atualizada para **v1.48.0** nos 3 pontos visuais (`index.html`, login e HUD).

### v1.47.1 — 25/09/2026

Correção Física de Colisões, Wall-Sliding e Camadas 2.5D com Translucidez X-Ray na «Zona Zero» (Bioma de Gelo):

- **Eliminação Total do Rubberbanding / Desync ("Personagem voltando no local")**:
  - Investigada e sanada a divergência crítica onde o cliente permitia penetrar árvores e obstáculos porque não encadeava as colisões da Zona Zero no `colideMapaAtivo`, enquanto o servidor barrava o movimento e forçava o recuo da posição a cada frame.
  - Sincronização estrita de `colideMapaAtivo` no cliente e `colideZonaZero` no servidor, com normalização de tipos (`circle`/`circulo` e `rect`/`caixa`/`box`).
- **Deslize Suave pelas Paredes (Wall-Sliding com Separação de Eixos)**:
  - Adicionada verificação de colisão por eixos independentes (`checarColisaoPonto`) na movimentação contínua de PC e Mobile.
  - Ao caminhar ou deslizar diagonalmente com inércia de gelo contra troncos de pinheiros, rochas ou bordas montanhosas, o jogador agora desliza suavemente ao longo do eixo desimpedido em vez de travar bruscamente ou tremer na colisão.
- **446 Colisões Físicas Sólidas e Fixas (`colisoes_zonazero.json`)**:
  - Reconstruído o catálogo físico do mapa com 446 obstáculos estáticos, todos com parâmetros numéricos válidos e zero campos `null`:
    - 247 Pinheiros Siberianos com colisão circular no tronco (`r = 22px`).
    - 100 Rochas Glaciais com raio de colisão esférico (`r = 20px`).
    - 88 Cristais e Geodos Eternos com colisão sólida no solo (`r = 16px`).
    - 4 Tendas de Expedição com colisão retangular sólida (`w = 90px, h = 60px`).
    - 4 Iglus de Caçadores com colisão circular sólida (`r = 45px`).
    - 3 Fósseis Congelados de Mamutes com base retangular rígida (`w = 120px, h = 60px`).
- **446 Camadas 2.5D com Translucidez X-Ray Inteligente (`camadas_zonazero.json`)**:
  - Inseridas 446 camadas de profundidade com ordenação dinâmica (`baseY` e `ordem: 1`) para z-sorting perfeito.
  - Efeito X-ray inteligente com atenuação suave de transparência (`globalAlpha = 0.48` sob copas de pinheiros e `globalAlpha = 0.52` sob tetos de tendas e cúpulas de iglus), mantendo visibilidade total do personagem.
- **Desobstrução do Ponto de Retorno e Spawn**:
  - Raio de segurança garantido de 70px livre de qualquer obstáculo no spawn seguro da expedição (`74200, 1000`) e no portal rúnico de retorno a Davahl (`74160, 1000`).
  - Reduzida a margem de colisão da borda montanhosa de 5 para 2 tiles, desimpedindo o tráfego nas colunas 2 e 3 do mapa.
- **Blindagem no Dash**:
  - O `usarDash` agora faz raycast contínuo em 16 passos via `colideMapaAtivo`, impedindo que o jogador termine dentro de obstáculos ou atravesse pinheiros no bioma de gelo.

### v1.47.0 — 25/09/2026

Novo Mapa Oficial: Bioma de Gelo «Zona Zero» (8.000 × 9.000 px) com Física de Deslize no Gelo e Efeitos Visuais Atmosféricos:

- **Dimensões e Terreno Glacial Procedural (8.000 × 9.000 px / 200×225 tiles de 40px)**:
  - Localizado entre as coordenadas mundiais `X ∈ [74000, 82000]` e `Y ∈ [0, 9000]`.
  - Terreno dividido em 5 biotipologias de piso com ruído Simplex e interpolação: Neve Fofa (`neve_fofa`), Neve Compacta batida (`neve_compacta`), Gelo Liso de Alta Fricção Reduzida (`gelo_liso`), Permafrost com veios azulados (`permafrost`) e Falésias de Rocha de Borda Glacial (`rocha_borda`).
- **Física Realista de Deslize no Gelo (Ice Sliding & Inertia)**:
  - Função detectora `ehGeloZonaZero(wx, wy)` analisa se as coordenadas do personagem coincidem com superfícies de gelo liso.
  - Ao entrar no gelo, a fricção é drasticamente reduzida: o personagem acelera com ganho suave (`0.16`) e desacelera com inércia (`0.95`), permitindo deslizar e manobrar em curvas fluidas.
  - Animação de passadas mais cadenciada durante o deslizamento e rastros visuais de patinação.
  - Suporte completo no teclado (PC - WASD/Setas) e no analógico virtual/touch (Mobile).
- **Efeitos Visuais e Atmosfera Glacial de Alta Fidelidade**:
  - **Aurora Borealis Dinâmica**: Cortinas de luz translúcidas ondulando no topo dos céus árticos em transições harmônicas verde-esmeralda, ciano glaciar e violeta cósmico.
  - **Nevasca Volumétrica 3D (Blizzard)**: 120 partículas de neve e flocos de gelo com oscilações sinusoidais de vento e velocidade variável simulando profundidade.
  - **Partículas de Brilho Cintilante (Ice Sparkles)**: Partículas em forma de diamante reluzindo sobre a superfície lisa dos lagos de gelo.
  - **Geodos e Cristais Eternos Luminescentes**: Agrupamentos de cristais de gelo facetados com pulsação de luz interna e halos cianos/azuis.
  - **Fogueira Polar com Iluminação Dinâmica**: Pira de sobrevivência no acampamento inicial emitindo luz dinâmica quente com flutuação de brasas e faíscas.
- **8 Complexos Temáticos e Pontos de Interesse (POIs)**:
  - *Posto da Expedição Zero*: Área de surgimento seguro (`x: 74200, y: 1000`) guarnecida com barracas de sobrevivência, fogueira e o Portal Rúnico de Retorno para Davahl (`74160, 1000`).
  - *Grande Espelho de Gelo*: Vasta extensão lacustre congelada ideal para deslizamento contínuo em alta velocidade.
  - *Santuário dos Cristais Eternos*: Centro de poder místico repleto de monólitos de gelo e cristais cintilantes.
  - *Aldeia Ártica de Caçadores*: Assentamento com iglus de blocos de gelo e equipamentos de pesca glacial.
  - *Vale dos Fósseis Congelados*: Esqueletos colossais de mamutes ancestrais e arcabouços de costelas fossilizadas.
  - *Fissura Abissal Azul*: Cânion glacial profundo com fendas de permafrost azulado.
  - *Cidadela Glacial Soterrada*: Ruínas de pedra ancestral consumidas pelo gelo eterno.
  - *Covil do Gelo Profundo*: Recanto isolado no extremo do mapa com agulhas de gelo afiadas e atmosfera gélida intensa.
- **Colisões Físicas Sólidas (435 Obstáculos) e Camadas 2.5D (255 Sortables)**:
  - 280 Pinheiros Siberianos com colisão circular nos troncos e efeito X-ray inteligente (`globalAlpha = 0.48`) ao andar por trás das copas nevadas.
  - 100 Rochas e Falésias Glaciais sólidas bloqueando passagens intransponíveis.
  - 70 Cristais Eternos com colisão física.
  - 12 Tendas e Iglus com paredes colidíveis e telhado com X-ray (`0.52`).
  - Matriz espacial 2D `colisoesGrid` particionada em tiles de 40px com checagem O(1) (<0.001ms) para manter 60 FPS inabaláveis.
- **Integração Global Multi-Mapas e Persistência**:
  - Menu de teleporte do HUD atualizado com botão rápido: `❄️ Zona Zero (Bioma de Gelo)`.
  - Suporte completo no Editor de Colisões e Camadas (`colisao-editor.js`), persistindo em `colisoes_zonazero.json` e `camadas_zonazero.json`.
  - Servidor autoritativo: validação de fronteiras (`WORLD_WIDTH = 82000`), balística de projéteis (`colideProjetilZonaZero`), teleporte e validação de pets.
  - Versão **v1.47.0** sincronizada nos 3 pontos visuais mandatórios.

### v1.46.1 — 25/09/2026

Correção Física de Colisões e Camadas 2.5D com Translucidez X-Ray no Pântano:

- **Colisões Físicas no Pântano (475 Obstáculos)**:
  - Adicionadas colisões sólidas para 10 Cabanas temáticas (paredes sólidas com varandas/passarelas transitáveis livres), 150 Ciprestes Ancestrais (troncos circulares), 130 Salgueiros-Chorões (troncos circulares), 80 Troncos Mortos/Caídos (segmentos com raio de colisão) e 100 Rochas de Musgo, impedindo que o jogador atravesse árvores e paredes.
  - Spawn do teleporte (`x: 50200, y: 1000`) protegido contra qualquer colisão física.
  - Balística e projéteis (`colideProjetilPantano`) bloqueados por paredes de cabana e troncos.
- **Grid Espacial 2D O(1) (<0.001ms)**:
  - Grade espacial `colisoesGrid` particionada em tiles de 40px no `mapa_pantano.js` para checagem imediata a 60 FPS sem alocação em loop.
- **Camadas 2.5D e Z-Sorting para Cabanas**:
  - Cabanas divididas em 2 subcamadas — `cabana_base` (piso/palafitas em `cb.y + cb.h * 0.45` para o jogador pisar sobre a varanda sem afundar) e `cabana_telhado` (telhado e vigas em `cb.y + cb.h + 8` para sobreposição correta).
- **Translucidez X-Ray Inteligente**:
  - Efeito X-ray inteligente com `globalAlpha = 0.52` ao passar sob telhados de cabanas e `globalAlpha = 0.48` ao caminhar atrás das copas de ciprestes e salgueiros, garantindo visibilidade total do personagem.
- **Persistência JSON & Editor**:
  - Arquivos `colisoes_pantano.json` (475 obstáculos com nomes) e `camadas_pantano.json` (170 camadas) gerados e prontos para visualização e edição no Editor.
- **Distance Culling no Servidor e Cliente**:
  - Otimização para checagem de colisões customizadas com raio de 160px.

### v1.46.0 — 25/09/2026

Editor de Colisões e Editor de Camadas Multi-Mapas (Suporte a Todos os 8 Mapas do Jogo):
- Suporte global no Editor a todos os 8 biomas: `cidade`, `green`, `desert`, `pantano`, `caverna`, `arena`, `cidadeperdida` e `testevisual`.
- Seletor dropdown temático na UI com badges dinâmicos de contagem de obstáculos e camadas, detecção automática do mapa onde o jogador está e botão para sincronizar com o bioma atual.
- Persistência dedicada por bioma em `colisoes_<mapa>.json` e `camadas_<mapa>.json` com coordenadas relativas locais (`x0, y0`).
- Colisão em tempo real autoritativa no servidor e no cliente com Z-sorting dinâmico de camadas.

### v1.45.0 — 25/09/2026

Replicação e Expansão do Bioma de Pântano Realista no Mapa Oficial da Fase 3 (Pântano das Almas, 8.000 × 9.000 px):

- **Terreno e Autotiling de Pântano 200×225 Tiles (8.000 × 9.000 px)**:
  - Solo rico e sombrio em lodo escuro com poças de decomposição (`lodo`), turfa vegetal densa (`turfa`), e áreas de lama movediça traiçoeira (`lama_movedica`).
  - 12 sistemas de lagos e lagoas de águas pantanosas venenosas (`veneno`) cobertas por lentilhas-d'água esmeralda (*duckweed*) e colônias flutuantes de vitórias-régias com flores aquáticas brancas e rosadas.
  - Mecânica de debuff de veneno preservada caso o jogador pise na água desprotegido.
- **8 Complexos Temáticos de Cabanas Arruinadas & Lagos**:
  1. *Posto das Brumas do Norte (`x: 50200, y: 1000`):* Local do spawn oficial de teleporte; passarela segura de madeira conectando ao continente; choça de observação arruinada com caibros quebrados e barris podres no lodo.
  2. *Vilarejo Palafita Submerso (`x: 52600, y: 2400`):* Grande lagoa pantanosa com complexo de 3 cabanas arruinadas (Cabana Comunitária, Armazém Podre e Píer) interligadas por passarelas em zigue-zague sobre palafitas inclinadas com telhados desabados.
  3. *Refúgio da Bruxa do Pântano / Santuário do Eremita (`x: 55800, y: 1900`):* Lagoa isolada envolta por grossos festões de barba-de-velho, cabana retorcida presa em raízes antigas e cogumelos bioluminescentes.
  4. *Atracadouro do Pescador Perdido (`x: 52000, y: 4500`):* Lagoa junto ao Portal de Retorno a Davahl; choça inclinada sobre estacas no brejo com redes de pesca rasgadas penduradas e canoa de madeira podre semi-submersa.
  5. *Feitoria Abandonada do Brejo Central (`x: 54800, y: 4600`):* Cabana sobre palafitas com varanda desmoronada, caixotes partidos e barris podres na água turva.
  6. *Cemitério de Barcos das Águas Negras (`x: 56400, y: 6400`):* Enseada profunda com 2 canoas naufragadas, estacas isoladas emergindo da lâmina d'água e choça inclinada.
  7. *Posto Avançado da Garganta Ocidental (`x: 50500, y: 8200`):* Na chegada do desfiladeiro do Deserto (`y: 8000..8600`), com passarela sobre o braço de lago venenoso e choça arruinada dos batedores.
  8. *Acampamento dos Coletores de Turfa do Sul (`x: 53600, y: 7800`):* Lagoa sul com choça de teto desabado, ferramentas esquecidas e passarelas contornando o brejo.
- **Passarelas de Madeira Podre (Boardwalks)**:
  - Trilhas rústicas de tábuas de madeira carcomidas e pregadas sobre estacas cravadas na lama, permitindo travessia limpa e estratégica sobre a água venenosa sem receber dano de veneno.
- **Centenas de Árvores com Z-Sorting Real 2.5D**:
  - 150 Ciprestes-Calvos Ancestrais com base alada (*buttressed trunk*) e raízes aéreas ("joelhos de cipreste") emergindo do lodo e da água.
  - 130 Salgueiros-Chorões com festões pendentes de barba-de-velho (*Spanish Moss*) balançando suavemente ao vento.
  - 80 Troncos Mortos Ocos caídos no lodo com cogumelos orelha-de-pau (*shelf fungi*).
  - Mais de 200 tufos densos de taboas (*cattails*) nas margens das lagoas.
- **Sistema Animado de Vagalumes Vivos (*Fireflies*)**:
  - 100 vagalumes procedurais autônomos que sobrevoam o pântano em órbitas tridimensionais suaves e aleatórias.
  - Animação de piscar dinâmico com ciclo orgânico e halos bioluminescentes (verde-limão, esmeralda e dourado suave) iluminando a escuridão do pântano.
- **Efeitos Atmosféricos**:
  - Bolhas de gás metano que brotam do lodo, sobem na lagoa e estouram liberando vapor tênue.
  - Névoa rasteira volumétrica translúcida cruzando a superfície do brejo.
- **Otimização Dual PC & Mobile**:
  - Frustum Culling rigoroso em todos os tiles, entidades de Z-sort e vagalumes, garantindo 60 FPS cravados no PC e no Mobile.
  - Versão **v1.45.0** atualizada na tela de login, HUD in-game e metadados.

### v1.44.0 — 25/09/2026

Transformação da Arena Visual em Bioma de Pântano Realista com Cabanas de Madeira Destruídas:

- **Terreno e Autotiling de Pântano Realista**:
  - Matriz completa de 32×24 tiles (1280×960 px) com lodo úmido escuro (`lodo`), turfa vegetal (`turfa`), lama movediça escura e escorregadia (`lama_movedica`), e musgo pantanoso (`musgo_pantanoso`).
  - Lagoas e canais de águas pantanosas profundas e turvas (`agua_turva`, `agua_funda`) com reflexos sombrios, lentilhas-d'água esmeralda (*duckweed*) e colônias flutuantes de vitórias-régias com flores aquáticas brancas e rosadas.
- **Passarelas de Madeira Podre (Boardwalks)**:
  - Trilhas rústicas de tábuas de madeira envelhecidas e carcomidas (`boardwalk`) apoiadas sobre estacas cravadas na lama, conectando o ponto de chegada do portal às cabanas e margens seguras.
- **Cabanas de Madeira Destruídas e Arruinadas**:
  - **Cabana Principal Arruinada**: Construção sobre palafitas de madeira inclinadas e podres; estrutura desabada com caibros e vigas de madeira expostos; tábuas partidas e soltas nas paredes; varanda de madeira carcomida tombada na lama; barris podres rachados e cobertos de musgo.
  - **Choça Menor do Pescador**: Choça inclinada sobre estacas no brejo com redes de pesca rasgadas penduradas, amarras velhas e uma canoa de madeira podre semi-submersa na água turva.
- **Vegetação Pantanosa com Z-Sorting Real 2.5D**:
  - **Ciprestes-Calvos Ancestrais (*Taxodium distichum*)**: Troncos alargados na base (*buttressed base*) com raízes aéreas protuberantes ("joelhos de cipreste") emergindo da água e da lama ao redor.
  - **Salgueiros-Chorões Pantanosos**: Troncos inclinados e copas com festões pendentes de barba-de-velho (*Spanish Moss*) que balançam suavemente ao vento.
  - **Troncos Mortos Ocos e Caídos**: Troncos apodrecidos tombados no lodo com cogumelos-de-orelha-de-pau (*shelf fungi*).
  - **Tufos Densos de Taboas (*Cattails*)**: Taboas esguias com espigas marrons e folhas longas crescendo nas bordas das águas turvas.
  - **Cogumelos Bioluminescentes**: Pequenos aglomerados de fungos luminosos emitindo pulsação suave em tons esmeralda e turquesa no pé dos ciprestes e madeiras podres.
- **Efeitos Atmosféricos e VFX Vivos**:
  - **Bolhas de Gás Metano**: Bolhas que brotam do fundo do lodo, sobem até a superfície da lagoa, estouram e liberam um tênue véu de vapor fétido.
  - **Névoa Rasteira Volumétrica**: Camadas de névoa rasteira em deslocamento sutil sobre as águas e a lama.
  - **Fogos-Fátuos Dançantes (*Will-o'-the-wisps*)**: Luzes etéreas flutuantes com movimento oscilante e partículas bioluminescentes sobre a lagoa.
- **Portal de Retorno Temático**:
  - Portal esculpido em arco de cipreste antigo com runas de musgo brilhante em verde esmeralda retornando a Davahl.
- **Compatibilidade Dual PC & Mobile**:
  - Renderização otimizada a 60 FPS com Frustum Culling e zero alocações por frame.
  - Versão **v1.44.0** atualizada na tela de login, HUD e metadados.

### v1.43.2 — 25/09/2026

Remoção Total dos Picos/Formações Rochosas ("Espinhos") e Expansão de Cabanas Nômades ao Redor de Todos os Lagos:

- **Remoção de 100% das Montanhas e Rochas do Deserto**:
  - A pedido do usuário, todas as formações rochosas de cânions e montanhas ('mont') foram completamente retiradas de todo o mapa do deserto (`mapa_deserto.js`).
  - O terreno do deserto agora é 100% aberto, plano e amplo, formado por suaves dunas onduladas com *ripple marks*, caminhos batidos de caravanas e solo árido rachado, proporcionando movimentação fluida e visual limpo e imersivo.
- **5 Grandes Oásis Cristalinos**:
  - Adicionado o **Oásis Escondido das Areias Rubras** (`cx: 510, cy: 350`), totalizando 5 grandes lagos límpidos com águas turquesa cristalinas, reflexos animados, margens úmidas e tamareiras esguias.
- **12 Assentamentos de Cabanas e Acampamentos Nômades (10 nas Margens dos Lagos)**:
  - Cada oásis agora conta com cabanas e acampamentos nômades integrados às suas margens, preservando o leito d'água sem sobrepor a lâmina do lago:
    - *Oásis 1 (Poço Ocidental):* Feitoria Nômade (Leste) + Cabana dos Pescadores (Noroeste).
    - *Oásis 2 (Grande Oásis Central):* Aldeia Real do Grande Oásis (Norte) + Acampamento dos Mercadores (Sudeste) + Refúgio Beduíno (Oeste).
    - *Oásis 3 (Tamareiras do Norte):* Posto Nômade (Leste) + Cabana dos Coletores de Tâmaras (Sul).
    - *Oásis 4 (Dunas do Sul):* Acampamento Beduíno (Norte) + Cabana do Vigia das Águas (Leste).
    - *Oásis 5 (Areias Rubras):* Feitoria do Oásis das Areias Rubras (Nordeste).
    - *Rotas:* Caravançarai da Fronteira (Oeste) + Acampamento Guardião da Garganta (Leste).
  - Tendas com lonas de linho em paletas exclusivas para cada região (linho cru, carmim do deserto, índigo escuro, azul turquesa, verde tâmara e dourado imperial), fogueiras com luz dinâmica, ânforas de cerâmica e tapetes ornamentais.
- **Compatibilidade Dual PC & Mobile**:
  - 60 FPS garantidos com frustum culling e zero alocações na renderização.
  - Versão **v1.43.2** aplicada na tela de login, HUD in-game e metadados.

### v1.43.1 — 25/09/2026

Substituição dos Picos Triangulares ("Espinhos") por Mesas de Arenito Contínuas e Estratificadas no Bioma de Deserto:

- **Remoção dos "Espinhos" / Triângulos Repetitivos por Tile**:
  - Eliminado o renderizador legado em `desenharMontanhaArenito` que desenhava triângulos pontiagudos isolados com fendas verticais em cada tile unitário de 40×40 (`lineTo(x + TILE/2, ty + 4)`).
  - Em clusters 3×3 ou áreas de montanhas, esses triângulos individuais criavam um padrão de grelha pontiaguda que parecia estacas, armadilhas ou espinhos artificiais no solo do deserto.
- **Renderização Monolítica Contínua com Autotiling Geológico**:
  - Implementado sistema de vizinhança topológica 4-direcional (Norte, Sul, Leste, Oeste) que detecta a continuidade de blocos rochosos adjacentes.
  - Conexão contínua entre blocos de arenito sem descontinuidades internas ou pontas isoladas: múltiplos tiles vizinhos agora formam um bloco geológico único, como platôs ou mesas de arenito realistas (estilo cânions do Arizona / Grand Canyon / Monument Valley).
  - Faixas horizontais de estratificação sedimentar mineral contínua em camadas de tonalidades terracota, ocre e arenito avermelhado, correndo horizontalmente sem quebras entre blocos contíguos.
  - Iluminação superior de crista na borda exposta ao sol (Norte) com realce dourado suave (`#d4895a`).
  - Sombra de escarpa e parede íngreme na borda inferior (Sul) com tom escuro profundo (`#5c2415`), além de sombra projetada suave no chão arenoso (`rgba(0, 0, 0, 0.28)`).
- **Otimização Dual PC & Mobile**:
  - Mantida alta performance a 60 FPS com zero alocação de objetos em loop (`mapa_deserto.js`).
  - Compatibilidade total em desktops e dispositivos móveis.
  - Versão atualizada para **v1.43.1** na tela inicial de login, HUD em jogo e metadados.

### v1.43.0 — 25/09/2026

Replicação e Expansão do Bioma de Deserto Realista em todo o mapa principal do Deserto (Fase 2, coordenadas `[18000, 50000)` × `[0, 36000)`):

- **Terreno e Relevo de Dunas 800×900 Tiles**:
  - Dunas de areia dourada com ondulações de vento (*ripple marks*) contínuas, gradientes quentes e cristas ensolaradas.
  - Cânions de arenito avermelhado na borda leste e formações rochosas internas com estratificação sedimentar e fissuras de erosão.
  - 4 Grandes Oásis de águas cristalinas azul-turquesa com marolas, reflexos e margens de areia úmida fértil.
  - Rede de caminhos batidos de caravanas ligando o portal de entrada ocidental até o desfiladeiro do pântano no leste.
  - Áreas de solo árido de argila ressecada com fendas e cascalho.
- **Vegetação e Cactos com Z-Sorting Real (2.5D)**:
  - Cactos Saguaro gigantescos canelados com nervuras verticais 3D (luz e sombra), múltiplos braços e flores.
  - Cactos Palma forrageira (Prickly Pear) com figos-da-índia vermelhos (*tunas*) e cactos bola/barril espinhosos.
  - Tamareiras esguias curvadas com frondes compridas arqueadas balançando na brisa e cachos de tâmaras ao redor de todos os oásis.
  - Z-Sorting preciso: jogador caminha livremente na frente ou por trás de troncos e copas com profundidade real.
- **5 Acampamentos Nômades Beduínos Estratégicos**:
  - Caravançarai da Fronteira (próximo à entrada do portal verde), Posto das Águas Ocidentais, Aldeia do Grande Oásis, Posto Avançado do Norte e Posto Guardião da Garganta Leste.
  - Tendas de tecido estilo beduíno com lonas de linho/pelo de camelo, listras terracota e índigo, mastros de madeira e cordas com estacas fincadas.
  - Tapetes geométricos carmim e ouro na entrada, esteiras de palha, caixotes de suprimentos, ânforas de cerâmica com água e lanternas de latão.
  - Fogueiras nômades rústicas com caldeirões sobre tripé de ferro, brasas incandescentes, labaredas, fumaça ao vento e luz dinâmica âmbar.
- **Ossadas e Esqueletos Colossais de Feras**:
  - 7 sítios de ossadas fósseis com crânios colossais de chifres curvados, grandes arcabouços de costelas arqueadas emergindo de dunas e ossos longos dispersos.
- **Ruínas Antigas e Santuários de Arenito**:
  - 5 sítios arqueológicos monumentais com colunas caneladas de arenito, capitéis clássicos esculpidos, arquitraves e tambores de coluna tombados na areia.
- **Elementos Extras Solicitados**:
  - 🌾 **Plantas-do-Deserto Rolantes (*Tumbleweeds*):** 8 bolas de arbustos secos saltitando e rolando ao vento através das dunas.
  - 🦅 **Sombras de Urubus Planando:** Silhuetas de aves de rapina planando alto em órbitas circulares sob o sol, projetando sombras nas dunas.
  - 🌪️ **Tornados de Vento Procedurais (*Dust Devils*):** 6 redemoinhos e tornados de poeira autônomos que surgem, erguem partículas em espiral 3D e vagam pelas areias com brisa contínua.
- **Versão Atualizada:** `v1.43.0` no login, HUD in-game e `GAME_VERSION`.

### v1.42.0 — 25/09/2026

Transformação da Arena Visual (Teste Gráfico) em um Bioma de Deserto Realista completo:

- **Terreno e Dunas de Areia Realistas**:
  - Dunas douradas com ondulações de vento dinâmicas (*ripple marks*) esculpidas com gradientes e cristas iluminadas pelo sol.
  - Caminhos de caravana batidos ligando o portal solar ao acampamento e ao oásis, com textura de terra compactada e marcas de passagem.
  - Paredes e cânions de arenito com estratificação geológica avermelhada e fissuras verticais de erosão.
  - Áreas de solo árido de argila ressecada com rachaduras e cascalho do deserto.
  - Oásis de águas límpidas azul-turquesa com reflexos solares, ondas cintilantes e margem arenosa úmida.
- **Vegetação e Cactos com Z-Sorting Real**:
  - Cactos Saguaro gigantescos canelados com nervuras verticais 3D sombreadas, múltiplos braços ascendentes, flores desérticas brancas/amarelas no topo e espinhos.
  - Cactos Palma forrageira (Prickly Pear) com figos-da-índia vermelhos (tunas) e cactos bola/barril com nervuras e flores.
  - Tamareiras esguias com troncos escamosos curvados, frondes compridas arqueadas balançando ao vento e cachos de tâmaras.
  - Z-Sorting 2.5D preciso: o jogador caminha na frente ou atrás dos cactos e árvores com sombreamento e profundidade natural.
- **Acampamento Nômade com Cabanas Beduínas**:
  - Tenda principal e tenda do mercador com lonas de linho/pelo de camelo esticadas, listras decorativas em terracota e índigo, mastros de madeira e cordas de fixação com estacas cravadas na areia.
  - Tapete oriental rústico com bordados geométricos estendido na entrada da tenda.
  - Ânforas de barro cozido com água, esteiras de palha, caixotes de suprimentos e lanternas de latão oscilando na brisa.
  - Fogueira nômade com caldeirão sobre tripé de ferro, brasas incandescentes, labaredas dinâmicas, fumaça ao vento e luz dinâmica âmbar.
- **Ossadas e Esqueletos de Feras Mortas**:
  - Crânio colossal com grandes chifres curvados, cavidades orbitais e mandíbula semi-enterrada na areia.
  - Arcabouço de costelas arqueadas de fera monstruosa emergindo de uma duna, projetando sombras no relevo.
  - Vértebras e fragmentos de ossos longos ressecados pelo sol dispersos pelo terreno.
- **Tornados de Vento Procedurais (Dust Devils)**:
  - Sistema dinâmico e autônomo com múltiplos tornados e redemoinhos de poeira vagando pelo deserto.
  - Ciclo de vida completo: surgimento com nuvem de poeira no solo, vórtice em cone invertido girando em alta velocidade, partículas e detritos rodopiando em espiral 3D e dissipação suave.
  - Partículas ambientais de poeira e grãos de areia suspensos cruzando o mapa na brisa quente.
- **Portal Solar e Interface**:
  - Portal de retorno renovado como Monólito Solar com anel rúnico dourado giratório e vórtice de areia para Davahl.
  - Minimapa atualizado com rótulo "🏜️ Arena Visual (Deserto)" e cor de fundo temática `#4a3915`.
  - Versão atualizada para **v1.42.0** no login, HUD in-game e `GAME_VERSION`.
