...
|---|---|---|---|
| **v1.38** | 23/09/2026 (hora local) | **JANELA DE HABILIDADES COM MOLDURA OFICIAL `SkillList.png` (1046×1504):** a arte oficial `imagem/HUD/SkillList.png` agora é a moldura da janela de Skills — técnica **"PNG por último"** (CAMADA 3, `pointer-events:none` na PNG), conteúdo dinâmico atrás (z-index 5) aparecendo pelos interiores transparentes da moldura, SEM fundo sólido e SEM redesenhar a interface. Container responsivo `width: min(90vw, calc(94vh * 1046/1504))` + `aspect-ratio: 1046/1504` mantendo a proporção em PC e mobile. `renderizarSkills()` adaptada SEM duplicar lógica (mesmo `SKILLS_INFO`, `skillsNiveis`, upgrades, dano/cura/mana/cooldown/servidor): **topo** = 1 único ícone de classe via `window.minhaClasse` (fotos `imagem/HUD/Perfil/*.png?v=perfil2`); **esquerda** = lista compacta por skill (ícone, nome, nível, categoria); **direita** = INFORMAÇÕES da skill selecionada (estado `window._skillSelecionada`, seleção clicável); **PRÓXIMO NÍVEL** (lógica atual de evolução) e **REQUISITOS** (vazio quando não há) abaixo; **rodapé** = barra de 8 slots com as skills da classe; botão **MELHORAR** mantém `melhorarSkill(id)`. Geometria encaixada nos vãos transparentes da PNG (respeitando as bandas douradas das labels): pontos 7%/17.5%, lista 6.2%/20.5% h58%, info 49.8%/18% h34%, PRÓXIMO NÍVEL 54.5%, REQUISITOS 68%, barra 6%*/87.2% (88%×10.4%), X 2.4%/3.4%, alça de arraste 20%→80% top 8%. Janela continua arrastável com posição persistida (drag & drop migrado para `#skills-png-wrap`/`#skills-drag-handle` em dragdrop.css/js). Cache-busters: `SkillList.png?v=HUDSKILLS01`, `skills.css?v=144`, `skills.js?v=146`. Validação visual por render GDI+ (3 rodadas de ajuste fino). Versão **v1.38** nos 3 pontos (login, HUD, `GAME_VERSION`) | `index.html`, `skills.css`, `skills.js`, `dragdrop.css`, `dragdrop.js`, `imagem/HUD/SkillList.png`, `CHANGELOG.md`, `INFO_PROJETO.md`, `REGRAS_IA.md` |
...
**JANELA DE HABILIDADES COM MOLDURA OFICIAL `SkillList.png` — a interface de Skills agora usa a arte oficial como moldura (overlay "PNG por último").**

- **Moldura `SkillList.png` (1046×1504):** a janela de Habilidades deixa de ser uma caixa CSS genérica e passa a ser a arte oficial `imagem/HUD/SkillList.png`, pintada **por último** (CAMADA 3, `z-index: 10`) por cima do conteúdo dinâmico (`#skills-content`, `z-index: 5`) — os interiores transparentes da moldura deixam o conteúdo aparecer; a PNG tem `pointer-events: none` para os cliques passarem para os elementos interativos;
- **Container responsivo:** `#skills-png-wrap` com `width: min(90vw, calc(94vh * 1046/1504))` e `aspect-ratio: 1046/1504` — a proporção da moldura se mantém em PC e mobile em qualquer resolução/redimensionamento;
- **`renderizarSkills()` adaptada (sem duplicar lógica):** reutiliza `SKILLS_INFO`, `skillsNiveis`, `NIVEL_SKILL_MAX` e todos os sistemas de upgrade (dano/cura/mana/cooldown/níveis/servidor intactos). Novos elementos apenas distribuem a informação na geometria da moldura:
  - **Topo:** 1 único ícone de classe via `window.minhaClasse` (fotos `imagem/HUD/Perfil/<classe>.png?v=perfil2`) no círculo central;
  - **Esquerda:** lista compacta por skill — uma linha por habilidade (ícone, nome, nível, categoria), com seleção clicável guardada em `window._skillSelecionada`;
  - **Direita:** painel INFORMAÇÕES da skill selecionada (nome, categoria, descrição, dano/cura/MP/cooldown, alcance/área, extra);
  - **PRÓXIMO NÍVEL** (54.5%): evolução real da lógica atual (valores com seta, bônus, aviso de máximo);
  - **REQUISITOS** (68%): exibição vazia quando a skill não tem requisitos ("Sem requisitos para evoluir");
  - **Rodapé:** barra de 8 slots (`6%/87.2%`, largura 88%, altura 10.4%) exibindo as skills da classe em ordem (1..8), com destaque para a selecionada;
  - **Botão MELHORAR** continua chamando `melhorarSkill(id)`; botão de reset da skill individual e resetar pontos também presentes.
- **Geometria encaixada nos vãos transparentes** (percentuais sobre 1046×1504, validados por 3 rodadas de render GDI+): pontos 7%/17.5%, lista 6.2%/20.5% (altura 58%), info 49.8%/18% (altura 34%), PRÓXIMO NÍVEL 49.8%/54.5% (altura 9.4%), REQUISITOS 49.8%/68% (altura 10.6%), barra 6%/87.2% (88%×10.4%), X 2.4%/3.4%, alça de arraste `left:20%; right:20%; top:8%; height:5%`;
- **Drag & drop mantido:** a janela continua arrastável e com posição persistida — o sistema foi migrado da antiga `#skills-window`/`#skills-header` para `#skills-png-wrap`/`#skills-drag-handle` (dragdrop.css linhas 6/41–50 e dragdrop.js linhas ~363/365/472 e 5541 do index.html);
- **Cache-busters:** `imagem/HUD/SkillList.png?v=HUDSKILLS01`, `skills.css?v=144`, `skills.js?v=146`;
- Versão do jogo: **v1.38** nos 3 pontos (login, HUD e `GAME_VERSION`).
